...
async fn measure_disk(path: &str) -> time::Duration {
    ...
    let future1 = tokio::spawn(async move { receive_value_disk(file2).await });

    let future2 = tokio::spawn(async move { send_value_disk(file1, 0).await });
...
async fn measure_channel() -> time::Duration {
    ...
    let f1 = tokio::spawn(async move { receive_value_channel(receiver).await });

    let f2 = tokio::spawn(async move { send_value_channel(sender, 0).await });
