        ...
        for result in tcp_listener.incoming() {
            ...
            loop {
                let bytes_read = stream
                    .read(&mut buf)
                    .expect("unable to read bytes from stream");
                if bytes_read == 0 {
                    break;
                }
    ...
    thread::spawn(move || {
        ...
        loop {
            let (bytes_read, src_addr) =
                udp_listener.recv_from(&mut buf).expect("no data received");
            if bytes_read == 0 {
                break;
            }
...
fn network_streams(c: &mut Criterion) {
    let mut group: criterion::BenchmarkGroup<'_, criterion::measurement::WallTime> =
        c.benchmark_group("network_streams");
    ...
    for scale in 0..7 {
        ...
        let mut stream =
            net::TcpStream::connect("127.0.0.1:36913").expect("couldn't connect to remote addr");
        group.bench_with_input(BenchmarkId::new("Tcp", n_values), &n_values, |b, i| {
            b.iter(|| tcp_send_values(&mut stream, n_values))
        });

        let mut socket =
            net::UdpSocket::bind("127.0.0.1:36915").expect("unable to bind to address");
        // Connect to udp server address
        socket
            .connect("127.0.0.1:36914")
            .expect("unable to connect to remote address");
        group.bench_with_input(BenchmarkId::new("Udp", n_values), &n_values, |b, i| {
            b.iter(|| udp_send_values(&mut socket, n_values))
        });
    }
    group.finish()
}
